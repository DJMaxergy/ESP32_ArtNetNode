/*
  ArtNetNode v3.1.0
  Copyright (c) 2024, DJMaxergy
  https://github.com/DJMaxergy/ESP32_ArtNetNode
...
#define CONFIG_VERSION "310"
#define FIRMWARE_VERSION "3.1.0"
#define ART_FIRM_VERSION 0x0310   // Firmware given over Artnet (2 uint8_ts)
...
struct StoreStruct {
  ...
  true,                        // dhcpEnable
  true,                        // standAloneEnable

  "esp32ArtNetNode",           // nodeName
  "esp32ArtNetNode DMX/Pixel interface", // longName
  "",                          // wifiSSID
  "",                          // wifiPass
  "esp32ArtNetNode",           // hotspotSSID
  "ArtNet32",                  // hotspotPass
  15,                          // hotspotDelay

  TYPE_DMX_OUT,                // portAmode
  TYPE_DMX_OUT,                // portBmode
...
void loop(void) {
  ...
  if ((statusTimer < millis()) && (deviceSettings.doFirmwareUpdate == false)) {
    // Flash status LEDs
    ...
    } else if (((isHotspot == true) && (WiFi.softAPgetStationNum() > 0) && (webServer.client().available() == false)) ||
               ((isHotspot == false) && (WiFi.status() == WL_CONNECTED) && (webServer.client().available() == false))) {
      statusStrip.SetPixelColor(ADDR_STATUS_LED_S, green);
    } else if (webServer.client().available() == true) {
...
static bool ajaxSave(uint8_t page, DynamicJsonDocument& json) {
  ...
  switch (page) {
    ...
    case 2:     // Wifi
      ...
      strncpy(deviceSettings.hotspotSSID, json["hotspotSSID"], 22);
    ...
    case 4:     // Port A
      {
        ...
        if (newMode == TYPE_SERIAL_LED && !doReboot) {
          ...
          if (newLen <= lim && oldLen != newLen) {
            ...
            // pixDriver.updateStrip(1, deviceSettings.portAnumPix, deviceSettings.portApixConfig); // Obsolete -> ToDo: Alternative needed?
    ...
    case 5:     // Port B
      {
        ...
        if (newMode == TYPE_SERIAL_LED && !doReboot) {
          ...
          if (newLen <= lim && oldLen != newLen) {
            ...
            // pixDriver.updateStrip(1, deviceSettings.portBnumPix, deviceSettings.portBpixConfig); // Obsolete -> ToDo: Alternative needed?
...
static void ajaxHandle() {
  ...
  DynamicJsonDocument jsonReply(2048);
  ...
  if (jsonDocument.containsKey("success") && jsonDocument["success"] == 1 && jsonDocument.containsKey("doUpdate")) {
    ...
    if (jsonDocument["doUpdate"] == 1) {
      ...
      pixPortA->ClearTo(black);
      pixPortB->ClearTo(black);
...
static void portSetup() {
  ...
  if (deviceSettings.portAmode == TYPE_DMX_OUT || deviceSettings.portAmode == TYPE_RDM_OUT) {
    ...
    pixPortA = new NeoPixelBus<NeoGrbFeature, NeoEsp32Rmt0Ws2812xMethod>(deviceSettings.portAnumPix, PIN_PORT_A);
    pixPortA->Begin();
    if (deviceSettings.portApixMode != FX_MODE_PIXEL_MAP) {
  ...
  if (deviceSettings.portBmode == TYPE_DMX_OUT || deviceSettings.portBmode == TYPE_RDM_OUT) {
    ...
    pixPortB = new NeoPixelBus<NeoGrbFeature, NeoEsp32Rmt1Ws2812xMethod>(deviceSettings.portBnumPix, PIN_PORT_B);
    pixPortB->Begin();
    if (deviceSettings.portBpixMode != FX_MODE_PIXEL_MAP) {
...
static void wifiStart() {
  ...
  if (strcmp(deviceSettings.hotspotSSID, "esp32ArtNetNode") == 0 || deviceSettings.hotspotSSID[0] == '\0') {
    sprintf(deviceSettings.hotspotSSID, "esp32ArtNetNode_%05u", uint32_t((ESP.getEfuseMac() >> 32) & 0xFFFF));
  }
