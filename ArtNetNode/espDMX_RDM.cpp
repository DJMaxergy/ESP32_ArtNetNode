...
//   dmx_set_baud_rate(dmx->dmx_nr, baud_rate);
// }
...
void espDMX::begin(uint8_t dir, uint8_t* buf) {
  if (_dmx == 0) {
    ...
    _dmx->data1 = (uint8_t*) malloc(sizeof(uint8_t) * DMX_PACKET_SIZE);
    memset(_dmx->data1, 0, DMX_PACKET_SIZE);
    ...
    _dmx->dmx_nr = _dmx_nr;
    _dmx->txPin = (_dmx->dmx_nr == 0) ? 1 : /*2*/16;
    _dmx->state = DMX_STOP;
    _dmx->txChan = 0;
  ...
  if (_dmx) {
    ...
    dmx_driver_install(_dmx->dmx_nr, &config, personalities, personality_count);

    /* Now set the DMX hardware pins to the pins that we want to use and setup
      will be complete! */
    dmx_set_pin(_dmx->dmx_nr, _dmx->txPin, 3, _dmx->dirPin);
  }
}
...
void espDMX::pause() {
  ...
  // digitalWrite(_dmx->dirPin, HIGH);
  dmx_driver_disable(_dmx->dmx_nr);
}
...
void espDMX::handler() {
  ...
  if (_dmx->state == DMX_STOP && _dmx->started) {
    ...
    for (int i = 1; i < DMX_PACKET_SIZE; i++) {
      _dmx->data1[i] = _dmx->data[i - 1];
    }
    dmx_write(_dmx->dmx_nr, _dmx->data1, _dmx->txSize);
    dmx_send_num(_dmx->dmx_nr, _dmx->txSize);
